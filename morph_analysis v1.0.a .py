...
def database_update(res_arr, theme): #Создание/Подключение к БД, создание новой таблицы и сохранение в ней результатов
    try:
...
def parsing(char, theme, c): #Парсинг всех комбинаций 
    res_arr = []
    for i in range(c):
        res_arr.append('')
    def func(m, i):
    ...
    return(database_update(res_arr, theme))
...
def check(): #Проверка валидности заполнения полей
    ...
    if len(input_text.get()) == 0:
        ...
        if err == 1:
            ...
            e_text.config(text = 'Данные в обработке', fg = '#009900')
            answer = mb.askyesno(
                title="Сокранение", 
...
def windowInit(): # Инициализация рабочего окна
    master = tk.Tk()
    master.title("Морфологический анализ v1.0")
